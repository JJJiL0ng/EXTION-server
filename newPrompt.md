# 수정된 Excel Formula Advanced 프롬프트

## 문제점 분석
기존 프롬프트에서 JavaScript 멀티셀 처리 시 `javascript/` 접두사 규칙이 명확하지 않고, 실행 가능한 완전한 코드 형태로 응답하지 않는 문제가 있었습니다.
...
**🎨 스타일링 작업:**
- var style = new GC.Spread.Sheets.Style(); style.backColor = '#FFFF00'; style.foreColor = '#000000'; worksheet.setStyle(row, col, style, GC.Spread.Sheets.SheetArea.viewport)
...
- spreadjsCommand: "worksheet.setFormula(56, 2, '=SUM(C2:C56)', GC.Spread.Sheets.SheetArea.viewport)" (단일 명령)
...
### 1. **javascript/ 접두사 규칙 명확화**
- 멀티셀 처리 시 반드시 `javascript/` 접두사 사용
- 단일 셀 처리 시에는 접두사 없이 직접 명령어 사용

### 2. **완전한 실행 코드 예시 추가**
- 기존의 불완전한 코드 조각 대신 바로 실행 가능한 완전한 코드 제공
- for 루프를 포함한 완전한 JavaScript 코드 구조

### 3. **구체적인 사용 사례 예시**
- "모든 고객 이름 가운데에 * 표시" 요청에 대한 완벽한 응답 예시 추가
- 다양한 멀티셀 처리 시나리오별 코드 예시

### 4. **에러 방지 코드 패턴**
- 데이터 존재 여부 확인 (`name && name.length > 1`)
- 타입 안전성 확인 (`typeof value === 'number'`)

### 5. **명확한 금지사항과 허용사항**
- ❌ 잘못된 예시와 ✅ 올바른 예시 구분
- 불완전한 코드 조각 vs 완전한 실행 코드

이 수정된 프롬프트를 사용하면 AI가 JavaScript 멀티셀 처리 요청에 대해 올바른 형태의 실행 가능한 코드를 반환할 것입니다.