...
SpreadJS 공식 전문가로서 다음을 포함하여 답변해주세요:
...
// 공식 설정 예시
sheet.setFormula(row, col, "=SUM(A1:A10)");

// 결과 확인
var result = sheet.getValue(row, col);
\`\`\`

실무에서 바로 적용 가능한 명확하고 실용적인 답변을 제공해주세요.
`,
...
데이터 분석 및 파이썬 전문가로서 다음을 포함하여 코드를 생성해주세요:
...
실행 가능하고 최신 파이썬 라이브러리를 활용한 코드를 제공해주세요.
pandas, numpy, matplotlib, seaborn 등 데이터 분석에 적합한 라이브러리를 활용하세요.
`,
    variables: ['dataContext', 'question']
  },
...
데이터 처리 전문가로서 다음을 포함하여 답변해드리겠습니다:
1. 현재 데이터 구조 분석
2. 요청에 맞는 데이터 변환 방법
3. 단계별 수정 프로세스
4. 데이터 정합성 검증 방법
...
전체 데이터를 효과적으로 수정하는 방법을 제공해드리겠습니다.
`,
    variables: ['dataContext', 'question']
  },
...
초보자도 이해할 수 있도록 친절하고 명확하게 설명해드리겠습니다.
`,
    variables: ['dataContext', 'question']
  }
];
