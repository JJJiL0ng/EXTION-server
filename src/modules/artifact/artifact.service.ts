...
export class ArtifactService {
  ...
  async generateArtifact(dto: GenerateArtifactDto): Promise<ArtifactResponseDto> {
    try {
      ...
      if (sheetsData && sheetsData.sheets && sheetsData.sheets.length > 0) {
      ...
      } else if (dto.extendedSheetContext) {
        this.logger.log('ExtendedSheetContext를 사용한 데이터 처리');
        // 기존 extendedSheetContext 처리 로직은 유지
      } else {
        ...
        this.logger.log(`dto.extendedSheetContext: ${JSON.stringify(dto.extendedSheetContext, null, 2)}`);
      }
      ...
      // === 3. 사용자 메시지 저장 ===
      const firebaseSheetContext = this.createFirebaseSheetContext(spreadsheetMetadata, activeSheetData);
      const userMessageDto: CreateMessageDto = {
        content: dto.userInput,
        role: MessageRole.USER,
        type: MessageType.ARTIFACT,
        mode: MessageMode.ARTIFACT,
        ...(firebaseSheetContext && { sheetContext: firebaseSheetContext }),
      };
  ...
  // === Firebase용 시트 컨텍스트 생성 ===
  private createFirebaseSheetContext(spreadsheetMetadata: any, activeSheetData: any): any {
  ...
  private selectSheetContext(dto: GenerateArtifactDto): ExtendedSheetContext | SheetContext | null {
    // 우선순위: extendedSheetContext > sheetContext > currentData > sheetsData의 활성 시트
    if (dto.extendedSheetContext) {
    ...
    // currentData 또는 sheetsData에서 활성 시트 찾기
    const sheetsData = dto.currentData || dto.sheetsData;
    if (sheetsData && sheetsData.sheets && sheetsData.sheets.length > 0) {
      // 활성 시트 찾기
...
- 총 시트 수: ${dto.extendedSheetContext?.totalSheets || (dto.currentData || dto.sheetsData)?.sheets?.length || 1}
` : `
### 단일 시트 환경:
...
**중요**: JSX를 절대 사용하지 말고, 모든 요소를 React.createElement로 생성해주세요. 코드 안에서 사용되는 표에 관한 단어들을 입력받는 데이터에 작성되어 있는대로 작성해주세요. 이렇게 해야 프론트엔드에서 오류 없이 렌더링됩니다.`;
  ...
  private extractHeaders(dto: GenerateArtifactDto): string[] {
    ...
    // currentData 또는 sheetsData에서 헤더 추출
    const sheetsData = dto.currentData || dto.sheetsData;
    if (sheetsData && sheetsData.sheets && sheetsData.sheets.length > 0) {
      const activeSheet = sheetsData.sheets.find(sheet => sheet.name === sheetsData.activeSheet);
  ...
  private createUserPrompt(userInput: string, dto: GenerateArtifactDto): string {
    ...
    const getDataRange = (): string => {
      ...
      // currentData 또는 sheetsData에서 데이터 범위 추출
      const sheetsData = dto.currentData || dto.sheetsData;
      if (sheetsData && sheetsData.sheets && sheetsData.sheets.length > 0) {
        const activeSheet = sheetsData.sheets.find(sheet => sheet.name === sheetsData.activeSheet);
    ...
    const getMultiSheetInfo = (): string => {
      ...
      if (dto.extendedSheetContext) {
        ...
        // currentData 또는 sheetsData에서 다중 시트 정보 추출
        const sheetsData = dto.currentData || dto.sheetsData;
        if (sheetsData && sheetsData.sheets) {
          info += `- **총 시트 수**: ${sheetsData.sheets.length}\n`;
          info += `- **시트 목록**: ${sheetsData.sheets.map(s => s.name).join(', ')}\n`;
          info += `- **활성 시트**: ${sheetsData.activeSheet}\n`;
        }
      }
...
**ComponentToRender 함수만 반환하고, 별도의 설명은 주석으로 포함해주세요.**`;
  ...
  private extractSampleData(dto: GenerateArtifactDto): string {
    ...
    if (dto.extendedSheetContext?.sampleData) {
      ...
      // currentData 또는 sheetsData에서 샘플 데이터 추출
      const sheetsData = dto.currentData || dto.sheetsData;
      if (sheetsData && sheetsData.sheets && sheetsData.sheets.length > 0) {
        const activeSheet = sheetsData.sheets.find(sheet => sheet.name === sheetsData.activeSheet);
