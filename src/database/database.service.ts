...
export class DatabaseService {
  ...
  /**
   * 어드민용: 권한 확인 없이 채팅 시트 데이터 조회
   */
  async getAdminChatSheetData(chatId: string): Promise<ChatSheetDataResponseDto> {
    this.logger.log(`어드민 채팅 시트 데이터 조회: chatId=${chatId}`);

    // Chat을 찾고 messages와 sheetMetaDataId를 함께 조회 (권한 확인 우회)
    const chat = await this.prisma.chat.findUnique({
      where: { id: chatId },
      include: {
        messages: {
    ...
    if (!chat) {
      this.logger.warn(`어드민 채팅을 찾을 수 없음: chatId=${chatId}`);
      throw new Error(`Chat with id ${chatId} not found`);
    }

    this.logger.log(`어드민 채팅 조회 완료: chatId=${chatId}, 메시지 수=${chat.messages.length}, userId=${chat.userId}`);
    ...
    // sheetMetaDataId가 없으면 sheetMetaData 없이 응답
    if (!chat.sheetMetaDataId) {
      this.logger.log(`어드민 시트 메타데이터 없음: chatId=${chatId}`);
      return {
        chatId: chat.id,
        chat: chatData,
        sheetMetaData: undefined,
    ...
    this.logger.log(`어드민 시트 메타데이터 조회 중: chatId=${chatId}, sheetMetaDataId=${chat.sheetMetaDataId}`);
    ...
    if (!sheetMetaData) {
      this.logger.warn(`어드민 시트 메타데이터를 찾을 수 없음: sheetMetaDataId=${chat.sheetMetaDataId}`);
      throw new Error(`SheetMetaData with id ${chat.sheetMetaDataId} not found`);
    }

    this.logger.log(`어드민 채팅 시트 데이터 조회 완료: chatId=${chatId}, 메시지 수=${chat.messages.length}, 테이블 수=${sheetMetaData.sheetTableData.length}, userId=${chat.userId}`);
